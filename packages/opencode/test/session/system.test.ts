...
describe("session.system", () => {
  test("returns the Voryel prompt for every model", () => {
    const ids = [
    ...
    for (const id of ids) {
      ...
      expect(prompt).toContain("operating through Voryel")
      expect(prompt).not.toContain("{{MODEL_NAME}}")
    }
  })

  test("returns the Voryel prompt for official Kimi provider IDs", () => {
