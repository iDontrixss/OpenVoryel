...
const getCurrentUrl = () => {
  ...
  // Baked at build time: lets a production build talk to a fixed server
  // origin (e.g. local backend on :4096) instead of its own origin.
  const host = import.meta.env.VITE_OPENCODE_SERVER_HOST
  const port = import.meta.env.VITE_OPENCODE_SERVER_PORT
  if (host || port) return `http://${host ?? location.hostname}:${port ?? "4096"}`
